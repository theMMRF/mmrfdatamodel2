...
class CopyNumberVariationWorkflow(base.Node):
    ...
    _dictionary: Dict[str, Union[bool, str, List[str]]] = {'title': 'Somatic Copy Number Variation Workflow', 'namespace': 'https://gdc.cancer.gov', 'category': 'analysis', 'submittable': True, 'downloadable': True, 'description': 'Metadata for the Somatic Copy Number Variation pipeline used to estimate copy number changes from different molecular data sources.', 'required': ['submitter_id', 'type', 'copy_number_segments'], 'project': '*', 'program': '*', 'previous_version_downloadable': True}
    ...
    @psqlgraph.pg_property(str , enum=['GISTIC', 'GISTIC - Arm Level Copy Number', 'GISTIC - Copy Number Score', 'GISTIC - Extreme Copy Number Score', 'GISTIC - Focal Amplification', 'GISTIC - Focal Deletion', 'GISTIC - Mean Copy Number Score'])
