"""This module is for CoreMetadataCollection Node.

This module is generated by plaster 0.1.dev140+mmrf from gdcdictionary version master,
please do not modify manually.
"""

from typing import Any, Dict, List, Optional, Tuple, Union

import psqlgraph
from sqlalchemy.ext import hybrid
from sqlalchemy.orm import Session, query

from .helpers import (
    base,
    datetime_hooks,
    indexes,
    related_cases,
    versioned_nodes,
    versioning,
)


class CoreMetadataCollection(base.Node):
    __tablename__: str = "node_coremetadatacollection"

    # this field contains values of uniqueProperties
    __pg_secondary_keys: List[List[str]] = [['project_id', 'submitter_id']]

    # _defaults: default value for specified fields in the dictionary
    _defaults:  Dict[str, Union[bool, float, int, str]] = {'state': 'validated'}
    _dictionary: Dict[str, Union[bool, str, List[str]]] = {'title': 'Core Metadata Collection', 'namespace': 'https://gen3.org', 'category': 'administrative', 'submittable': True, 'description': 'A collection of data files in a project.', 'required': ['submitter_id', 'type', 'projects'], 'project': '*', 'program': '*'}

    _pg_backrefs: Optional[Dict[str, Dict[str, Union[str, psqlgraph.Node]]]] = None
    _pg_edges: Optional[Dict[str, Dict[str, Union[str, psqlgraph.Node]]]] = None
    _pg_links: Optional[Dict[str, Dict[str, Union[str, psqlgraph.Node]]]] = None

    @classmethod
    def get_label(cls) -> str:
        return "core_metadata_collection"

    @property
    def id(self):
        return self.node_id

    @id.setter
    def id(self, value):
        self.node_id = value

    @classmethod
    def pg_secondary_keys(cls) -> List[List[str]]:
        return cls.__pg_secondary_keys

    @classmethod
    def post_process(cls) -> None:
        cls.add_secondary_key_indexes()
        cls.populate_pg_backrefs()
        cls.populate_pg_links()
        cls.populate_pg_edges()

    @classmethod
    def add_secondary_key_indexes(cls) -> None:
        secondary_key_indexes = indexes.get_secondary_key_indexes(cls)
        for index in secondary_key_indexes:
            cls.__table__.append_constraint(index)

    @classmethod
    def populate_pg_backrefs(cls) -> None:
        """_pg_backrefs are in_edges, links FROM other types."""
        cls._pg_backrefs = {
                "aggregated_somatic_mutations": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("aggregated_somatic_mutation"),
                },
                "aligned_reads_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("aligned_reads"),
                },
                "aligned_reads_indexes": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("aligned_reads_index"),
                },
                "analysis_metadata_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("analysis_metadata"),
                },
                "annotated_somatic_mutations": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("annotated_somatic_mutation"),
                },
                "archives": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("archive"),
                },
                "biospecimen_supplements": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("biospecimen_supplement"),
                },
                "clinical_supplements": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("clinical_supplement"),
                },
                "copy_number_auxiliary_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("copy_number_auxiliary_file"),
                },
                "copy_number_estimates": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("copy_number_estimate"),
                },
                "copy_number_segments": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("copy_number_segment"),
                },
                "cytof_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("cytof"),
                },
                "experiment_metadata_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("experiment_metadata"),
                },
                "files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("file"),
                },
                "filtered_copy_number_segments": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("filtered_copy_number_segment"),
                },
                "gene_expressions": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("gene_expression"),
                },
                "germline_copy_number_auxiliary_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("germline_copy_number_auxiliary_file"),
                },
                "germline_copy_number_estimates": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("germline_copy_number_estimate"),
                },
                "germline_copy_number_segment": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("germline_copy_number_segment"),
                },
                "germline_mutation_indexes": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("germline_mutation_index"),
                },
                "germline_structural_variations": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("germline_structural_variation"),
                },
                "masked_methylation_arrays": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("masked_methylation_array"),
                },
                "masked_somatic_mutations": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("masked_somatic_mutation"),
                },
                "methylation_beta_values": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("methylation_beta_value"),
                },
                "mirna_expressions": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("mirna_expression"),
                },
                "pathology_reports": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("pathology_report"),
                },
                "protein_expressions": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("protein_expression"),
                },
                "raw_methylation_arrays": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("raw_methylation_array"),
                },
                "run_metadata_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("run_metadata"),
                },
                "secondary_expression_analyses": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("secondary_expression_analysis"),
                },
                "simple_germline_variations": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("simple_germline_variation"),
                },
                "simple_somatic_mutations": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("simple_somatic_mutation"),
                },
                "slide_images": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("slide_image"),
                },
                "somatic_mutation_indexes": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("somatic_mutation_index"),
                },
                "structural_variations": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("structural_variation"),
                },
                "submitted_aligned_reads_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("submitted_aligned_reads"),
                },
                "submitted_expression_arrays": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("submitted_expression_array"),
                },
                "submitted_genomic_profiles": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("submitted_genomic_profile"),
                },
                "submitted_genotyping_arrays": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("submitted_genotyping_array"),
                },
                "submitted_methylation_beta_values": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("submitted_methylation_beta_value"),
                },
                "submitted_tangent_copy_number": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("submitted_tangent_copy_number"),
                },
                "submitted_unaligned_reads_files": {
                    "name": "core_metadata_collections",
                    "src_type": base.Node.get_subclass("submitted_unaligned_reads"),
                },
        }

    @classmethod
    def populate_pg_edges(cls) -> None:
        """_pg_edges are all edges, links to AND from other types."""
        cls._pg_edges = {
                "aggregated_somatic_mutations": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("aggregated_somatic_mutation"),
                },
                "aligned_reads_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("aligned_reads"),
                },
                "aligned_reads_indexes": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("aligned_reads_index"),
                },
                "analysis_metadata_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("analysis_metadata"),
                },
                "annotated_somatic_mutations": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("annotated_somatic_mutation"),
                },
                "archives": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("archive"),
                },
                "biospecimen_supplements": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("biospecimen_supplement"),
                },
                "clinical_supplements": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("clinical_supplement"),
                },
                "copy_number_auxiliary_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("copy_number_auxiliary_file"),
                },
                "copy_number_estimates": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("copy_number_estimate"),
                },
                "copy_number_segments": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("copy_number_segment"),
                },
                "cytof_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("cytof"),
                },
                "experiment_metadata_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("experiment_metadata"),
                },
                "files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("file"),
                },
                "filtered_copy_number_segments": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("filtered_copy_number_segment"),
                },
                "gene_expressions": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("gene_expression"),
                },
                "germline_copy_number_auxiliary_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("germline_copy_number_auxiliary_file"),
                },
                "germline_copy_number_estimates": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("germline_copy_number_estimate"),
                },
                "germline_copy_number_segment": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("germline_copy_number_segment"),
                },
                "germline_mutation_indexes": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("germline_mutation_index"),
                },
                "germline_structural_variations": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("germline_structural_variation"),
                },
                "masked_methylation_arrays": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("masked_methylation_array"),
                },
                "masked_somatic_mutations": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("masked_somatic_mutation"),
                },
                "methylation_beta_values": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("methylation_beta_value"),
                },
                "mirna_expressions": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("mirna_expression"),
                },
                "pathology_reports": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("pathology_report"),
                },
                "projects": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("project"),
                },
                "protein_expressions": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("protein_expression"),
                },
                "raw_methylation_arrays": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("raw_methylation_array"),
                },
                "run_metadata_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("run_metadata"),
                },
                "secondary_expression_analyses": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("secondary_expression_analysis"),
                },
                "simple_germline_variations": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("simple_germline_variation"),
                },
                "simple_somatic_mutations": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("simple_somatic_mutation"),
                },
                "slide_images": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("slide_image"),
                },
                "somatic_mutation_indexes": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("somatic_mutation_index"),
                },
                "structural_variations": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("structural_variation"),
                },
                "submitted_aligned_reads_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("submitted_aligned_reads"),
                },
                "submitted_expression_arrays": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("submitted_expression_array"),
                },
                "submitted_genomic_profiles": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("submitted_genomic_profile"),
                },
                "submitted_genotyping_arrays": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("submitted_genotyping_array"),
                },
                "submitted_methylation_beta_values": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("submitted_methylation_beta_value"),
                },
                "submitted_tangent_copy_number": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("submitted_tangent_copy_number"),
                },
                "submitted_unaligned_reads_files": {
                    "backref": "core_metadata_collections",
                    "type": base.Node.get_subclass("submitted_unaligned_reads"),
                },
        }

    @classmethod
    def populate_pg_links(cls) -> None:
        """_pg_links are out_edges, links TO other types."""
        cls._pg_links = {
                "projects": {
                    "edge_out": "_CoreMetadataCollectionDataFromProject_out",
                    "dst_type": base.Node.get_subclass("project"),
                },
        }


    @property
    def _related_cases_from_cache(self) -> List[psqlgraph.Node]:
        return related_cases.get_related_cases_from_cache(self)

    @property
    def _related_cases_from_parents(self) -> List[psqlgraph.Node]:
        return related_cases.get_related_cases_from_parents(self)

    @property
    def _secondary_keys_dicts(self) -> List[Dict[str, Any]]:
        vals = []
        secondary_keys = self.__pg_secondary_keys
        for keys in secondary_keys:
            if "id" in keys:
                continue
            vals.append({key: getattr(self, key, None) for key in keys})
        return vals

    @hybrid.hybrid_property
    def _secondary_keys(self) -> Tuple[Tuple[Any]]:
        vals = []
        for keys in self.__pg_secondary_keys:
            vals.append(tuple(getattr(self, key) for key in keys))
        return tuple(vals)

    @_secondary_keys.comparator
    def _secondary_keys(cls):
        return indexes.SecondaryKeyComparator(cls)

    # Set this attribute so psqlgraph doesn't treat it as a property
    _secondary_keys._is_pg_property = False


    @property
    def _versions(self) -> query.Query:
        """Return a query to get node versions (node history).

        Returns a query if the node is bound to a session. Raises an
            exception if the node is not bound to a session. This is different
            from node tagging and versioning. This is used by sheepdog to save
            node history when a node is updated. But the data seems never read by
            any repo.

        Args:
            self: Psqlgraph Node

        Returns:
            A SQLAlchemy query for node versions.

        Raises:
            RuntimeError if the node is not bound to a session.
        """
        session = self.get_session()
        if not session:
            raise RuntimeError(f"{self} not bound to a session. Try .get_versions(session).")
        return self.get_versions(session)

    def get_versions(self, session: Session) -> query.Query:
        """Return a query for node versions given a session."""
        return (
            session.query(versioned_nodes.VersionedNode)
            .filter(versioned_nodes.VersionedNode.node_id == self.node_id)
            .filter(versioned_nodes.VersionedNode.label == self.label)
            .order_by(versioned_nodes.VersionedNode.key.desc())
        )


    @psqlgraph.pg_property(str )
    def submitter_id(self, value):
        self._set_property("submitter_id", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(int )
    def batch_id(self, value):
        self._set_property("batch_id", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str , enum=['error', 'invalid', 'live', 'md5summed', 'md5summing', 'redacted', 'released', 'submitted', 'suppressed', 'uploaded', 'uploading', 'validated', 'validating'])
    def state(self, value):
        self._set_property("state", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def project_id(self, value):
        self._set_property("project_id", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str, type(None) )
    def created_datetime(self, value):
        self._set_property("created_datetime", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str, type(None) )
    def updated_datetime(self, value):
        self._set_property("updated_datetime", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def contributor(self, value):
        self._set_property("contributor", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def coverage(self, value):
        self._set_property("coverage", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def creator(self, value):
        self._set_property("creator", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def data_type(self, value):
        self._set_property("data_type", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def date(self, value):
        self._set_property("date", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def description(self, value):
        self._set_property("description", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def format(self, value):
        self._set_property("format", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def language(self, value):
        self._set_property("language", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def publisher(self, value):
        self._set_property("publisher", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def relation(self, value):
        self._set_property("relation", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def rights(self, value):
        self._set_property("rights", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def source(self, value):
        self._set_property("source", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def subject(self, value):
        self._set_property("subject", value)  # type: ignore  # inherited from CommonBase


    @psqlgraph.pg_property(str )
    def title(self, value):
        self._set_property("title", value)  # type: ignore  # inherited from CommonBase



datetime_hooks.cls_inject_created_datetime_hook(CoreMetadataCollection)
datetime_hooks.cls_inject_updated_datetime_hook(CoreMetadataCollection)

